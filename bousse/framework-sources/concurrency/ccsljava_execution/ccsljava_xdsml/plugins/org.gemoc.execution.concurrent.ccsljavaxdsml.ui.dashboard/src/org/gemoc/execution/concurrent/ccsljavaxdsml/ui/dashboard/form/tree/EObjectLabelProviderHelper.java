...
import org.gemoc.execution.concurrent.ccsljavaxdsml.ui.dashboard.utils.ICommonConstants;
...
public class EObjectLabelProviderHelper {
	...
	protected static IItemLabelProvider getItemLabelProvider(EObject object_p) {
		...
		return (IItemLabelProvider)editingDomain.getAdapterFactory()
				.adapt(object_p, IItemLabelProvider.class);
	}
	...
	public static String getMetaclassLabel(EObject object_p, boolean addBrackets_p) {
		...
		// Adaptation to ItemProviderAdapter returns null due to EMF Edit generated ItemProviderAdapterFactory
		// that do not support this type.
	...
	/**
	 * Get the metaclass label (emitted by EMF Edit generation) for given object according given editing
	 * domain.
	 * 
	 * @param object_p
	 * @param provider_p
	...
	public static String getMetaclassLabel(EClass class_p, ItemProviderAdapter provider_p) {
		...
		try {
			label = provider_p.getString(GENERATED_KEY_PREFIX + class_p.getName()
					+ METACLASS_GENERATED_KEY_SUFFIX);
	...
	public static String getFeatureLabel(EStructuralFeature feature_p, ItemProviderAdapter provider_p) {
		...
		String featureKey = feature_p.getEContainingClass().getName() + ICommonConstants.UNDERSCORE_CHARACTER
				+ feature_p.getName();
